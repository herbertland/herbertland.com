...
class Board {
	...
	Board(int _max_cols, int _max_rows, int _xlen, int _ylen, Graphics g) {
		max_cols= _max_cols;
		max_rows= _max_rows;
		xlen= _xlen;
		ylen= _ylen;
...
{	
	...
	public void init() {
		...
		for (icount= 0; icount < 10; icount++) {
			param[icount]= getParameter("Image" + icount);
		...
		num_images= 0;
		while (icount > 0) {
			which= Math.abs(rand.nextInt()) % icount;
			if (load_image(param[which], num_images)) {
				num_images++;
			}
			icount--;
			param[which]= param[icount];
		}
	}
	...
	public void make_board(Image image)
	{
	   ...
	   */
	   ...
	   getGraphics().drawString("here is is " + max_cols + ":" + max_rows, 20, 120);
	   board= new Board(max_cols, max_rows, xlen, ylen, getGraphics());  
	...
	public boolean load_image(String urlstr, int ref)
	{	
		try {
	...
	public void run() {
		int which= 0;
		getGraphics().drawString("Num images= " + num_images, 20, 20);
		while (true) {
			try {
				stop();
			...
			if (sound != null) {
				...
				getGraphics().drawString("XXXXXXXXXXXXXStarting221", 20, 40);
			}
			make_board(images[which]);
			getGraphics().drawString("Starting331", 20, 40);
			board.digi_in();
			getGraphics().drawString("Starting441", 20, 20);
			delay(predelay);
